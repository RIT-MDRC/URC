...
// READING COMMAND FROM SERIAL ***********************************************************
   ...
   * Valid Command Identifiers: 
        ...
        E - Use end effector
   */
  // Check if command has been sent via Serial Terminal
  if (Serial.available()) {parseTerminalCommand();}

  // Check if command sent via Bluetooth
  if (Serial2.available()) {parseBluetoothCommand();}
